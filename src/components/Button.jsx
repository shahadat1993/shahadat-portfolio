 ...
 */
export default function Button({ to, href, onClick, variant = 'primary', children, className = '', type, disabled = false }) {
  ...
  if (to) return <Link to={to} data-cursor-hover onClick={onClick} className="inline-block">{content}</Link>;
  if (href) return <a href={href} target="_blank" rel="noreferrer" data-cursor-hover className="inline-block">{content}</a>;
  return <button type={type || 'button'} disabled={disabled} data-cursor-hover onClick={onClick} className="inline-block">{content}</button>;
}
